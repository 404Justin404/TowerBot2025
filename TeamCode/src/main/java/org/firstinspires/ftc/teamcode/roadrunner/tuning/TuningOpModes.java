...
public final class TuningOpModes {
    ...
    public static void register(OpModeManager manager) {
        ...
        if (DRIVE_CLASS.equals(MecanumDrive.class)) {
            dvf = hardwareMap -> {
                MecanumDrive md = new MecanumDrive(hardwareMap, FtcDashboard.getInstance().getTelemetry());
                ...
                if (md.localizer instanceof SmartLocalizer) {
                    SmartLocalizer dl = (SmartLocalizer) md.localizer;
                    encoderGroups.add(new LynxQuadratureEncoderGroup(
                            hardwareMap.getAll(LynxModule.class),
                            Arrays.asList(dl.parallelEncoder, dl.perpendicularEncoder)
                    ));
